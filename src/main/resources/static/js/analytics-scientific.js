...
let sessionCharts = new Map();
let timelineLoadingSessionId = null;
let trendSignature = null;
...
async function loadSessionTimeline(session) {
    ...
    if (String(timelineLoadingSessionId) === String(session.id)) return;
    timelineLoadingSessionId = session.id;
    if (!sessionCharts.size) { state.classList.remove("hidden"); state.textContent="Loading session readings…"; }
    try { const timeline=await request(`/api/monitoring-sessions/${session.id}/timeline`); if(String(selectedSessionId)!==String(session.id))return;
        if(!timeline.readings.length){destroySessionCharts();container.replaceChildren();state.classList.remove("hidden");state.textContent="No sensor readings were recorded during this session.";return;} state.classList.add("hidden");
        ...
        [...sessionCharts.keys()].filter(unit=>!groups.has(unit)).forEach(unit=>{const entry=sessionCharts.get(unit);entry.chart.destroy();entry.section.remove();sessionCharts.delete(unit);});
        groups.forEach((readings,unit)=>{const count=new Set(readings.map(item=>item.sensorId)).size;let entry=sessionCharts.get(unit);if(!entry){const section=document.createElement("section");section.className="scientific-session-chart";const heading=document.createElement("h3");heading.textContent=unit==="Value"?"Sensor values":unit;const holder=document.createElement("div");holder.className="scientific-session-canvas";const canvas=document.createElement("canvas");holder.append(canvas);section.append(heading,holder);container.append(section);entry={section,chart:LabMonitorSessionChart.create(canvas,timeline,readings,{showLegend:count>1})};sessionCharts.set(unit,entry);}else{LabMonitorSessionChart.update(entry.chart,timeline,readings,{showLegend:count>1});container.append(entry.section);}});
    } catch(error){state.textContent=error.message;}
    finally { if(String(timelineLoadingSessionId)===String(session.id)) timelineLoadingSessionId=null; }
}
function destroySessionCharts(){sessionCharts.forEach(entry=>entry.chart.destroy());sessionCharts.clear();}
...
document.addEventListener("labmonitor:refresh",async()=>{if(refreshInProgress||document.visibilityState!=="visible")return;refreshInProgress=true;try{await loadOverview(selectedOrganizationId,{silent:true});}finally{refreshInProgress=false;}});
